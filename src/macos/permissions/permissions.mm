// https://github.com/nut-tree/node-mac-permissions/blob/develop/src/permissions.mm
...
void AskForScreenCaptureAccess(const Napi::CallbackInfo &info) {
  // This is the modern API call for macOS 10.16+
  if (@available(macOS 10.16, *)) {
    CGRequestScreenCaptureAccess();
  }
  // The following block is a compile-time check.
  // It ensures the code inside is ONLY included when building with an SDK
  // older than macOS 15, where CGDisplayStreamCreate still existed.
#if defined(__MAC_OS_X_VERSION_MAX_ALLOWED) && __MAC_OS_X_VERSION_MAX_ALLOWED < 150000
  // This runtime check is now safely nested inside the compile-time check.
